...
object GPUImageFilterTools {
    ...
    val blendFliters = FilterList().apply {
    ...
    val blackFilters = FilterList().apply {
    ...
    val adjustableFilters = FilterList().apply {
...
//        addFilter("Grouped filters", FilterType.FILTER_GROUP)
    ...
    fun getFilterProgressInPercent(filterType: FilterType): Int {
        return when (filterType) {
            FilterType.BRIGHTNESS -> 50
            FilterType.WHITE_BALANCE -> 50
            FilterType.SOBEL_EDGE_DETECTION -> 50
            FilterType.BILATERAL_BLUR -> 100
            FilterType.VIGNETTE -> 100
            FilterType.GAMMA -> 50
            FilterType.LEVELS_FILTER_MIN -> 100
            FilterType.EXPOSURE -> 50
            FilterType.SATURATION -> 50
            FilterType.THRESHOLD_EDGE_DETECTION -> 50
            FilterType.RGB -> 100
            FilterType.CONTRAST -> 50
            FilterType.POSTERIZE -> 100
            FilterType.MONOCHROME -> 100
            FilterType.GLASS_SPHERE -> 40
            FilterType.SPHERE_REFRACTION -> 40
            FilterType.SHARPEN -> 50
            FilterType.LAPLACIAN -> 100
            FilterType.VIBRANCE -> 50
            FilterType.LUMINANCE_THRESHSOLD -> 50
            FilterType.HAZE -> 50
            FilterType.CROSSHATCH -> 2
            FilterType.MONOCHROME -> 0
            FilterType.NON_MAXIMUM_SUPPRESSION -> 100
            else -> 0
        }
    }
    fun showDialog(
        ...
        val filtersArr = filters.getNames().toTypedArray()
        builder.setItems(filtersArr) { _, pos ->
            listener(createFilterForType(context, filters.getFilter(filtersArr[pos])))
        }
        builder.create().show()
...
//                bitmap = BitmapFactory.decodeResource(context.resources, R.drawable.ic_launcher)
    ...
    class FilterAdjuster(filter: GPUImageFilter) {
        ...
        private inner class ContrastAdjuster(filter: GPUImageContrastFilter) :
            ...
            override fun adjust(percentage: Int) {
                filter.setContrast(range(percentage, 0.5f, 1.5f))
            }
        }
...
//                filter.setBrightness(range(percentage, -1.0f, 1.0f))
        ...
        private inner class ThresholdAdjuster(filter: GPUImageThresholdEdgeDetectionFilter) :
            ...
            override fun adjust(percentage: Int) {
                filter.setLineSize(range(percentage, 0.5f, 5.0f))
        ...
        private inner class ExposureAdjuster(filter: GPUImageExposureFilter) :
            ...
            override fun adjust(percentage: Int) {
                filter.setExposure(range(percentage, -2.0f, 2.0f))
            }
        }
        ...
        private inner class VignetteAdjuster(filter: GPUImageVignetteFilter) :
            ...
            override fun adjust(percentage: Int) {
                filter.setVignetteStart(range(percentage, 0.0f, 0.7f))
            }
        }
        ...
        private inner class BilateralAdjuster(filter: GPUImageBilateralBlurFilter) :
            ...
            override fun adjust(percentage: Int) {
                filter.setDistanceNormalizationFactor(range(percentage, -10.0f, 15.0f))
            }
        }
